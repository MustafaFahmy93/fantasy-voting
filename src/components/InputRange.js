...
const InputRange = ({ inputName, playerPower, disabled, method }) => {
    ...
    return (
        ...
        <div className={inputName === "OVERALL" ? "flex flex-col space-y-2 p-2 lg:w-full w-full" : "flex flex-col space-y-2 p-2 lg:w-1/3 w-full"}>
            <label className={inputName === "OVERALL" ? "font-bold text-gray-400 text-center" : "font-bold text-gray-100"}>{inputName}</label>
            <div className="inline-flex space-x-2">
                ...
                <input type="text" name={inputName + "-label"} className="w-2/12 text-center bg-transparent" value={playerPower} onChange={(e) => handleChange(e)} />
